...
public class LlmClient {
    ...
    public String chat(String systemPrompt, String userPrompt) {
        ...
        try {
            ...
            if (e.getCause() instanceof HttpStatusCodeException he) {
                int status = he.getStatusCode().value();
                String responseBody = he.getResponseBodyAsString();
                String msg = status == 401
                        ? "LLM API authentication failed (401). Check that OPENAI_API_KEY is set and valid when starting the server."
                        : "LLM API error (" + status + "): " + (responseBody != null && !responseBody.isBlank() ? responseBody : he.getMessage());
                throw new RuntimeException(msg, he);
            }
