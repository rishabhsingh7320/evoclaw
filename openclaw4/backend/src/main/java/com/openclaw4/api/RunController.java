package com.openclaw4.api;

import com.openclaw4.api.dto.RunDetailDto;
import com.openclaw4.api.dto.TaskDto;
import com.openclaw4.api.dto.TaskStepDto;
...
public class RunController {
    ...
    public ResponseEntity<RunDetailDto> getRun(@PathVariable Long runId) {
        ...
        detail.setParentRunId(run.getParentRunId());
    ...
    public ResponseEntity<Map<String, Object>> getDbSnapshot(@PathVariable Long runId) {
        ...
        runMap.put("sessionId", run.getSession().getId());
        runMap.put("rootMessageId", run.getRootMessage().getId());
        runMap.put("phase", run.getPhase());
        runMap.put("planJson", run.getPlanJson() != null ? run.getPlanJson() : "");
        runMap.put("parentRunId", run.getParentRunId() != null ? run.getParentRunId() : "");
        runMap.put("startedAt", run.getStartedAt() != null ? run.getStartedAt().toString() : "");
        runMap.put("finishedAt", run.getFinishedAt() != null ? run.getFinishedAt().toString() : "");
        snapshot.put("run", runMap);
        ...
        List<Memory> memories = memoryRepository.findBySessionOrderByUpdatedAtDesc(run.getSession());
        List<Map<String, Object>> memList = new ArrayList<>();
        for (Memory mem : memories) {
            Map<String, Object> mm = new LinkedHashMap<>();
            mm.put("id", mem.getId());
            mm.put("key", mem.getKey());
            mm.put("value", mem.getValue());
            mm.put("createdAt", mem.getCreatedAt().toString());
            mm.put("updatedAt", mem.getUpdatedAt().toString());
            memList.add(mm);
        }
        snapshot.put("memory", memList);
